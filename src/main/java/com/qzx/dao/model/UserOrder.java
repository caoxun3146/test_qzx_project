...
/*    @Column(name="request")
    ...
    @ManyToOne(cascade = {CascadeType.DETACH})   //CascadeType.DETACH   级联脱管/游离操作  ,如果你要删除一个实体，但是它有外键无法删除，你就需要这个级联权限了。它会撤销所有相关的外键关联。
    @JoinColumn(name="user_id",insertable=false,updatable=false)// 由于同一个字段注解两次报错，解决办法是让一个注解字段设成只读 insertable=false,updatable=false
